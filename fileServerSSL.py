...
import ssl
if (len(sys.argv) < 4):
    print("Usage: {} port certfile certkeyfile".format(sys.argv[0]))
    sys.exit(1)
...
if (not os.path.exists(certfile)):
    print("certficate file not exist: " + certfile)
    sys.exit(1)
if (not os.path.exists(keyfile)):
    print("certficate key file not exist: " + keyfile)
    sys.exit(1)
...
try:
    while True:
        ...
        try:
            tcpClient = ssl.wrap_socket(tcpClient, server_side=True,  
                        certfile=certfile, keyfile=keyfile, ssl_version=ssl.PROTOCOL_TLSv1_2)
            ...
            if (cmd == "#SENDFILE#"):
                ...
                tcpClient.send(b'#OK#\nsave file to {}'.format(filename).encode('utf8'))
